...
def init_database():
    ...
    if count == 0:
        print("🌱 Sembrando 50 eventos históricos...")
        seed_path = os.path.join(os.path.dirname(__file__), '..', '..', 'scripts', 'seed_events.json')
        with open(seed_path, 'r') as f:
            events = json.load(f)
        for evt in events:
            event = HistoricalEvent(
                year=evt['year'], epoch=evt['epoch'], event=evt['event'],
                solar_phase=evt['solar_phase'], estimated_excitability=evt['estimated_excitability'],
                description=evt['description']
            )
            session.add(event)
        session.commit()
        print(f"✅ {len(events)} eventos sembrados.")
